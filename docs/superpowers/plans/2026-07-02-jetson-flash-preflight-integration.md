...
- [ ] **Step 1.1:** `git add bin/jetson-flash-preflight.sh bin/check-emmc-pcn.sh && git commit -m "feat: add jetson-flash-preflight + check-emmc-pcn scripts as provided"`
...
- [ ] **Step 2.1: `bin/check-emmc-pcn.sh` — SKIP_CHECK marker.** After the L3 comment, insert:

```bash
# SKIP_CHECK — straight-line script with side effects; check_bash.yml must never source-execute it.
```

- [ ] **Step 2.2: preflight header.** L8 verdict line + L11 env line become:
...
- [ ] **Step 2.3: constants (after `REAPPLY_CMD`, L30).** Insert:
...
- [ ] **Step 2.4: guard `companion_via` (bug fix).** First body line becomes:
...
- [ ] **Step 2.5: `ensure_companion` (after `companion_via`, before section 1).**
...
- [ ] **Step 2.6: call it.** In the `7035)` branch insert `ensure_companion` directly after the `ok "state: FLASH INITRD …"` line; in the `7020)` branch insert `ensure_companion` directly after `booted_ok=0`.

- [ ] **Step 2.7: gate the interactive retry loop.** L99 condition gains a readability check:

```bash
        if [ "$booted_ok" != 1 ] && [ -t 0 ] && [ -r "$COMPANION" ]; then
```

- [ ] **Step 2.8: `$0` hint.** L118 becomes:

```bash
            warn "  re-run with JETSON_PASS='<password>' [JETSON_SSH=<user@host>]     # non-interactive"
```

- [ ] **Step 2.9: verdict exit codes.** The final case becomes:
...
- [ ] **Step 3.1:** Replace the existing `er_usb_camera_healthcheck()` block with:
...
- [ ] **Step 3.2: verify.** `bash -n .helper_bash_functions`; `bash -c 'source .helper_bash_functions; _fetch_and_call_remote_script bin/does-not-exist.sh; echo rc=$?'` → ERROR + rc=1; `bash -c 'source .helper_bash_functions; er_usb_camera_healthcheck --help; echo rc=$?'` → usage from the real main-branch script, rc=0; `bash -c 'source .helper_bash_functions; THIS_SCRIPT_BRANCH= er_usb_camera_healthcheck'` → branch-unset ERROR, rc=1.

- [ ] **Step 3.3: commit** `refactor: extract _fetch_and_call_remote_script; add er_jetson_flash_preflight`
...
- [ ] **Step 4.3: commit** `docs: add jetson-flash-preflight page + README entry`
...
- [ ] **Step 5.2: verify** `python3 -c "import yaml,sys; yaml.safe_load(open('.github/workflows/check_jetson_flash_preflight.yml'))"` → no output.
- [ ] **Step 5.3: commit** `ci: shellcheck jetson-flash-preflight scripts`

### Task 6: Verification sweep (pre-push)

- [ ] **Step 6.1:** `shellcheck bin/jetson-flash-preflight.sh bin/check-emmc-pcn.sh bin/usb-camera-healthcheck.sh` → clean.
- [ ] **Step 6.2:** `bash tests/test_usb_camera_healthcheck.sh` → "N passed, 0 failed".
- [ ] **Step 6.3:** Simulate `check_bash.yml`: run its grep loop locally; assert neither new bin script is matched (env-bash shebang) and every matched file sources cleanly.
- [ ] **Step 6.4:** Stub harness in the scratchpad (NOT committed): PATH shims for `lsusb`/`sudo`/`ssh`/`sshpass` + fake L4T tree with `flash_t234_qspi_sdmmc.xml` + fake `nvautoflash.sh` printing `Board ID(3701) version(501) sku(0004) revision(N.0)`. Assert: 7023+stock → DO NOT FLASH exit 1; 7023+patched → GO exit 0; no device+stock → exit 2; 7020 with `COMPANION=/nonexistent` and unfetchable branch → options warning, exit 2, output contains no "post-PCN" misread.

### Task 7: Push + end-to-end + PR

- [ ] **Step 7.1:** `git push -u origin jetson-flash-preflight`.
- [ ] **Step 7.2 (e2e):** `bash -c 'source .helper_bash_functions; THIS_SCRIPT_BRANCH=jetson-flash-preflight er_jetson_flash_preflight'` → real fetch from the branch; companion auto-fetch resolves on the branch (`ER_BUILD_TOOLS_BRANCH` passthrough); exit 2 on this dev box.
- [ ] **Step 7.3:** `gh pr create` — title `Jetson flash preflight`, body mirroring #43: intro, `## Setup` post-merge `.bashrc` snippet + pre-merge branch snippet (`ER_BUILD_TOOLS_BRANCH=jetson-flash-preflight bash <(curl -Ls …/refs/heads/jetson-flash-preflight/bin/jetson-flash-preflight.sh)`), `## Usage`, notes-for-review (companion auto-fetch + misread bug fix; exit-code contract is a deliberate addition; `er_usb_camera_healthcheck` refactored onto `_fetch_and_call_remote_script`, no behaviour change). Footer: 🤖 Generated with [Claude Code](https://claude.com/claude-code)

### Task 8: /review-pr + judged fixes

- [ ] **Step 8.1:** Run the `review-pr` skill on the new PR with two machine-specific adaptations: keep `GH_TOKEN` set (on this box gh is only authenticated through it), and inline the full `ponytail-review` ruleset (captured from the installed plugin) into the Ponytail Reviewer prompt.
- [ ] **Step 8.2:** Triage findings with judgment (fix what's worth fixing, skip with stated reason otherwise), commit, push.
- [ ] **Step 8.3:** notify-send + final summary for Tom's manual PR review.
